...
//            cfg inside a target that runs others. An `#[ignore]`d test is listed (`... ignored`), so it counts.
...
/** A test attribute: `#[test]`, `#[tokio::test]`, `#[tokio::test(flavor = …)]`. Not `wasm_bindgen_test`: that never
 * runs natively BY DESIGN and cargo's native run is not where it is counted. */
...
export function addedTests(diff) {
  ...
  for (const line of diff.split("\n")) {
    ...
    if (line.startsWith("+") && TEST_ATTR.test(body)) { pending = true; continue; }
    if (!pending) continue;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (cmd === "silent") {
    ...
    const u = unranTests(diff, readFileSync(arg, "utf8"));
    for (const t of u) process.stdout.write(`${t.file}: added test \`${t.name}\` never ran\n`);
    if (!u.length) process.stdout.write(`added tests: ${addedTests(diff).length}, all ran\n`);
