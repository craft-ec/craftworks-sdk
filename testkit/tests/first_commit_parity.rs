...
fn drain(c: &mut Conn, out: &mut Vec<Vec<u8>>) {
    while c.held() > 0 {
        out.extend(c.release_one());
    }
}

/// (what write 1 was told, PUTs after its publish, the most any call stranded)
fn first_commit(ticks_in_flight: bool) -> (Vec<WriteState>, usize, usize) {
...
fn the_first_commit_gets_its_parity_with_or_without_a_tick_in_flight() {
    for ticks in [true, false] {
        let (st, puts, stranded) = first_commit(ticks);
        println!("  ticks in flight: {ticks:<5}  PUTs after publish +{puts}  stranded {stranded}  told {st:?}");
        assert!(
        ...
        assert_eq!(
            stranded, 0,
            "ticks={ticks}: a call stranded {stranded} effect(s)"
        );
