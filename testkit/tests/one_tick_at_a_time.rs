...
/// Hand every tab's frames to the node; a frame the node REFUSES is answered
/// to its tab by the host error -- `frame_refused`, as the page's session does
/// on `Incoming::Refused` -- when `heard` is true.
fn offer_hearing_refusals(q: &mut NodeQueue, tabs: &mut [Tab], heard: bool) {
    for (i, t) in tabs.iter_mut().enumerate() {
        for f in t.store.take_outbound() {
            if !q.offer(i, f) && heard {
                t.store.client.frame_refused();
            }
        }
    }
}

/// A burst of writes against a parked delegate: the node refuses what its
/// queue cannot hold. Each refusal answers its frame, so once the delegate is
/// free the page's ticks go at once, one a second -- and the control, the
/// same refusals unheard, shows the gate shut for FORGET_MS (the architect's
/// sdk#196 review, 1b: 2 refused -> 22 ticks in 120 s).
#[test]
fn refused_frames_do_not_shut_the_tick_gate() {
    use craftworks_sdk::store::{Edit, Store};
    for heard in [true, false] {
        ...
        for i in 0..12u64 {
            tabs[0]
                .store
                .apply_batch(&[(format!("k/{i}").into_bytes(), Edit::Put(b"v".to_vec()))])
                .expect("taken");
        }
        offer_hearing_refusals(&mut q, &mut tabs, heard);
        let refused = q.rejected;
        q.park(false);
        serve(&mut q, &mut tabs);
        let before = ticks_of(&q.ran, 0).len();
        for _ in 0..30 {
            tabs[0].clock.advance(1000);
            let now = tabs[0].clock.now_ms();
            tabs[0].store.send_tick(now);
        ...
        let ran = ticks_of(&q.ran, 0).len() - before;
        println!("  refusals heard {heard}: {refused} refused, ticks run in the next 30 s: {ran}");
        assert!(refused >= 2, "the burst was not refused: the test is empty");
        if heard {
            assert_eq!(ran, 30, "refused frames held the tick gate shut");
        } else {
            assert!(ran <= 1, "CONTROL: unheard refusals did not shut the gate after the first tick ({ran} ran), so the test above measures nothing");
        }
    }
}
