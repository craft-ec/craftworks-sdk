...
struct Deadline {
    ...
    resent: bool,
    /// Its place in the node's ONE queue for this client (F61): the order it
    /// went on the wire. Many ops share a ms, so the clock cannot order them.
    seq: u64,
    /// Sent into an EMPTY queue: nothing was on the wire ahead of it, so its
    /// answer times the node's service plus the path -- the only answer that
    /// is an RTT sample (sdk#390). A queued op's answer also times its wait.
    alone: bool,
}

/// The wait of a FIRST send with `ahead` ops on the wire before it, at an RTO
/// of `rto`: the node answers one client's ops in one sequence (F61), so each
/// op ahead is one more RTO, capped at the RTO's ceiling (sdk#390, the
/// architect). THE one arming formula: at the send, and at every re-arm.
...
pub struct Page {
    ...
    /// The `seq` the next op put on the wire takes.
    next_send: u64,
...
impl Page {
    ...
    pub fn reconnected(&mut self, now: Ms) {
        self.now = now.0;
        // Re-sent in the order they first queued, and queued anew in it.
        let mut on_wire: Vec<(u64, Waiting)> = self.deadlines.iter().filter(|(_, d)| d.sent).map(|(w, d)| (d.seq, w.clone())).collect();
    ...
    fn send(&mut self, w: Waiting, op: Op) {
        ...
        // A FIRST send waits its place in the node's one queue (sdk#390, F61):
        // an RTO for each op on the wire ahead of it, and one for itself.
        let ahead = self.deadlines.values().filter(|d| d.sent).count() as u64;
    ...
    fn park(&mut self, w: Waiting, op: Op, attempt: u32) {
        ...
        let seq = self.next_seq();
        self.deadlines.insert(w, Deadline { at, armed: at, op, sent_at: self.now, attempt, sent: false, resent: false, seq, alone: false });
    }

    /// An ANSWER for `w`: its deadline ends, the answer to a first send into
    /// an empty queue is a sample (Karn: a re-sent call never is; sdk#390: a
    /// queued one also times its wait), every first send still on the wire is
    /// re-armed, and a GET opens the window. `None`: nothing was waiting on it.
    ...
    /// EVERY answer restarts the timer of every op still on its FIRST send
    /// from THIS answer, by the one arming formula at its place in the queue
    /// NOW, never later than the deadline it was sent with:
    /// `min(armed, now + queued_wait(ahead now))` (sdk#378 + sdk#390; RFC 6298
    /// §5.3, the architect).
    ///
    ...
    /// duplicate PUTs into the same queue. Timed from the answer at one RTO
    /// for every op (no place), an RTO sampled from an EMPTY queue (~100 ms)
    /// is shorter than the wait of an op a dozen places back. So an op waits
    /// an RTO for each op still ahead of it and one for itself, counted from
    /// the latest answer: nothing is lost while the queue keeps moving, and a
    /// lost op at the head goes one RTO after the last answer.
    ///
    /// ANY answer re-arms, a re-send's too: it is no sample, but it shows the
    /// path alive and the queue one shorter, and the `min` never extends a
    /// deadline. A RE-SEND keeps its backoff, and so does a reconnect re-send.
    /// One pass in queue order over the ops on the wire (a 300-row save has
    /// hundreds): the place is counted from the deadlines, never kept.
    fn rearm_first_sends(&mut self) {
        let now = self.now;
        let rto = self.rto.rto_ms();
        let mut wire: Vec<&mut Deadline> = self.deadlines.values_mut().filter(|d| d.sent).collect();
        wire.sort_unstable_by_key(|d| d.seq);
        for (ahead, d) in wire.into_iter().enumerate() {
            if d.attempt == 1 && !d.resent {
                d.at = d.armed.min(now + queued_wait(rto, ahead as u64));
            }
        }
    }
...
mod rto_queue {
    ...
    fn a_slow_head_is_re_sent_alone_and_the_op_behind_it_is_not() {
        ...
        println!("  rto after the first answer {rto_after_first}; re-sent (ms, put): {resent:?}");
        assert_eq!(resent, vec![(1_037 + rto_after_first, 2)], "not the slow head alone was re-sent");
    }
